...
for i in range(1000):
    ...
    for image in images:
        ...
        layer_1_output = relu(np.matmul(layer_1_matrix, input_vector) - bias_vector_1)
        # layer 2 output (16 x 1 matrix)
        layer_2_output = relu(np.matmul(layer_2_matrix, layer_1_output) - bias_vector_2)
        # output vector (10 x 1 matrix)
        output_vector = softmax(np.matmul(output_layer_matrix, layer_2_output) - bias_vector_output)
        print(output_vector)
        ...
        eps = 1e-12
        loss = -np.log(output_vector[labelled_number] + eps)    # cross-entropy
        print(loss)
    ...
    gradient_descent = np.mean(gradient_matrix, axis=0)
    ...
    bias_vector_2 += alpha * gradient_descent[16:32]
    bias_vector_output += alpha * gradient_descent[32:48]

    for i in range(len(layer_1_matrix)):
        lower_bound = 48+i*len(layer_1_matrix[i])
    ...
    for j in range(len(layer_2_matrix)):
        lower_bound = 48+len(layer_1_matrix)*len(layer_1_matrix[0])  +  j*len(layer_2_matrix[j])
    ...
    for k in range(len(output_layer_matrix)):
        lower_bound = 48+len(layer_1_matrix)*len(layer_1_matrix[0])+len(layer_2_matrix)*len(layer_2_matrix[0])  +  k*len(output_layer_matrix) 
